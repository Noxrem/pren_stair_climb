...
void SSD1306_InvertDisplay(bool invert){
  if(invert) SSD1306_WriteCommand(SSD1306_INVERT_DISPLAY);
  else SSD1306_WriteCommand(SSD1306_NORMAL_DISPLAY);
}

void SSD1306_SetColStartAddr(uint8_t col){
  actCol = col;
  if( actCol >= SSD1306_DISPLAY_HW_NOF_COLUMNS) return;
...
void SSD1306_SetPageStartAddr(uint8_t page){
  actPage = page;
  if(actPage >= SSD1306_DISPLAY_HW_NOF_PAGES) return;
...
void SSD1306_PrintChar(char ch){
  if(ch == '\n') {
    actPage++;
    if(actPage >= SSD1306_DISPLAY_HW_NOF_PAGES) actPage=0;
